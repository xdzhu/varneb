...
在本次路径投影审计完成时，余下材料级门槛是以最高像 15 为种子做局部全变量胞驻点精化，并在原子位移
...
这一步仍只是原生产链候选像的局部曲率；端点 Γ 原子模
与联合曲率不可混为同一个谱。

**2026-09-27 后续闭环，不回写历史分析：** 从该候选像单独建立
的 1000 eV 同协议局部诊断，已有去整体平移的中心梯度
`0.001557 eV/Å`、两个步长各一条且仅一条可信负曲率
`−4.03052/−4.04937 eV/Å²`，负方向与原链切线重叠
`0.997847`；沿正负方向的独立下坡分别进入经结构与静态
力/应力审核的 B4/B1 盆地。在同一 1000 eV 静态焓基准下，
正反势垒为 `0.338379/0.343221 eV/GaN`，而原 600 eV
生产链正向势垒为 `0.338491 eV/GaN`。详细审计见上引
联合曲率协议和
[`gan_b4_b1_1000eV_bilateral_index1_evidence_2026-09-27.json`](../benchmarks/numerical_integrity/gan_b4_b1_1000eV_bilateral_index1_evidence_2026-09-27.json)。
这一证据支持**1000 eV 局部近驻定指数一候选点及双盆地连接**，
仍不证明全局最低焓路径唯一；1000 eV 与 600 eV 的绝对焓
不可交叉相减，不能用本项诊断覆盖原路径的能量数据。
