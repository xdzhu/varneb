...
| GaN / ABACUS（新解析器重跑） | 27744907 | 运行中；已完成至第 3 步，`fmax=5.131 eV/A`；image 19 出现单像体积发散，当前作业仍保留以完成证据审计。 |

## 下一步

1. 等待并审计 `27744907` 的 ABACUS 生产结果；若再次失败，只看最小解析器的具体契约错误，不再改 FIRE 参数。
2. 后续新生产链启用 `maximum_cell_step`；当前 `27744907` 不修改运行中的源代码，等它生成更多证据后决定是否从安全快照独立续算。
