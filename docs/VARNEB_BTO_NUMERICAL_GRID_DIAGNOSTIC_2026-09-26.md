...
不是模式势能面的物理起伏。生产案例继续使用 PBE、100 Ry、
10 au DZP、`4×4×4` k 点；下面的 120 Ry 仅是诊断。
...
没有一致地改善。这组结果**不支持**“将 100 Ry 提至 120 Ry
即可解决小曲率力—能量一致性问题”，也不能用来认定真实误差
唯一来自 FFT 网格。无论如何，生产计算参数并未改动。
五点两套原始能量的精简公共数据表为
...
1. 不为追求亚 `0.01 meV/BTO` 的漂亮数字盲目提高 cutoff 或重复
   整张二维 DFT 网格；现有 59 点冻结切片可作为探索性图，但不是
   已认证的条件最小值面。
