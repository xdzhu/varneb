...
严格模式约束应定义活动子空间 `S=span{v_1,...,v_K}`，并将每轮广义更新投影到 `S`：
...
或在约化坐标中直接优化。对于质量加权模式，`V^TV` 必须替换为相应质量/扩展度量内积。该功能当前尚未完成，不能用 `mode_guided_path()` 冒充。

### 7.3 方向和原子约束

方向约束可以表示为每个原子的投影矩阵 `P_a`，或更一般的稀疏线性约束矩阵。它与 atom mask 的组合必须有冲突检测，并明确投影发生在 NEB 切向分解之前还是之后。默认方案将在有限差分和解析 toy model 验证后冻结。
