...
> BTO 横向材料验证的输入门禁和执行顺序见
> `docs/CROSS_BACKEND_BTO_PLAN.md`；在 QE UPF 与 LAMMPS 势函数明确前不提交材料级作业。
...
- [x] 验收：新节点上可用 `python -m pytest -q` 一条命令完成所有非 DFT 回归；失败时 pytest 日志能定位到具体检查脚本和输入。各 `tests/check_*.py` 仍保留独立命令入口。
...
- [x] calculator 异常、NaN/Inf、非法 cell、SCF/超时/MPI 关键词现在会保留带 `failure_category` 和恢复提示的 failure report；失败时会读取显式 calculator 目录下有限长度的 ABACUS/VASP 日志尾部来补充分类，并发 image 失败批次会保留已完成 sibling 结果。显式 `BFGSLineSearch` 的 `LineSearch failed!` 现在支持有界缩步重建式重试，其他异常不自动吞掉。
...
- [x] ABACUS command profile、32-MPI worker 宽度、Slurm 退出码和真实生产作业已验证；超时与 SCF 不收敛现有专用日志分类，显式 line-search 失败已有有界缩步重试，calculator 级重试仍由 `image_retries` 显式控制。
...
- [x] ABACUS/VASP 生产驱动现已暴露统一的 `--mode`、`--mode-guided`、`--constraint-mode {subspace,projected}` 及模式归一化选项；四个 Hefei Slurm 模板通过环境变量传递这些参数，默认仍为无约束 FIRE，未因接口补齐而启动 DFT。
