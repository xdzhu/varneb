...
- 集群规则：`235` 只作为登录网关；实际计算只在 `cu17`、`cu22`、`cu23`、`cu24`、`cu25`、`cu26` 上执行，每个节点 40 核，直接登录节点运行，不使用 `qsub` 或 PBS 调度，也不在本机 WSL 运行 DFT/NEB 长任务。
...
- [ ] 在指定节点之一重新运行现有回归集，优先使用 `cu17`，失败时换用 `cu22`；记录 Python、ASE、NumPy、SciPy、VASP/ABACUS 环境信息。
