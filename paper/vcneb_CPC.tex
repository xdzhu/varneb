...
refinement gives a $0.1292$~eV barrier.  A same-resolution linear-cell control
changes the ordinary seven-image barrier by only $0.00293$~eV while retaining
the same interior peak.  A BaTiO$_3$ tetragonal-to-cubic matrix
...
concludes with release availability, limitations, and the remaining validation
scope.
...
forces, and stress using VASP 6.3.2.  The ABACUS adapter also passed a
single-image HfO$_2$ calculator smoke test with finite energy, forces, and
stress.  These are calculator smoke tests, not convergence claims for a
phase-transition barrier.  All production material calculations reported
below use the 100~Ry, full 10~au DZP setup; the earlier low-cost smoke input is
retained only as historical adapter provenance.
...
under-resolved, non-converged control and was not used to report a barrier.  To
probe the cell metric independently of image count, a seven-total-image
linear-cell control (job 27678924) used the same endpoints and calculator
settings.  It converged to 0.04951~eV/\AA and a 0.1596772~eV barrier, only
0.0029263~eV above the logarithmic-strain seven-image path; the combined
linear/logarithmic comparison had no geometry or peak-location failure.
...
HfO$_2$ linear-cell control, 7 total images & $0.1596772$ eV barrier & Audited \\
...
standard workflow.  The linear-cell control quantifies a separate
parameterization sensitivity rather than being folded into the image-count
error.
