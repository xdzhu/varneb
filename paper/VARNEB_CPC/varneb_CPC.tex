...
restartable implementation with explicit numerical and provenance checks. We
also introduce a feasibility-aware local and staged optimization workflow:
image-scaled and block-local FIRE variants reduce calculator evaluations in
controlled BaTiO$_3$/HfO$_2$ ablations, while a pre-DFT cell gate provides
auditable local recovery for long VASP chains. These acceleration results are
reported as an implementation-level contribution, with negative transfer
cases retained to delimit the claim.
\end{abstract}
...
The quantitative material results therefore use both ABACUS/PBE and
VASP/PBE, with their calculator settings reported separately. The VASP backend
has exercised the complete distributed path, restart, and lattice-contract
workflow on GaN, HfO$_2$, and CdSe. The GaN tetragonal and hexagonal routes
agree with their published barrier scales, while a B3 route and the small
barrier CdSe route expose mapping and functional sensitivity. A working
backend is not treated as proof of cross-engine identity; each comparison is
reported with its pressure, formula-unit normalization, and literature method.
...
switch. These controls are separated from the physical path metric: they
...
The second part is a feasibility gate between the optimizer and the
...
Table~\ref{tab:acceleration} summarizes the first-principles ablation. The
rows share their starting chain, calculator settings, endpoints, image count,
spring, pressure, and $0.10$ eV/\AA{} stopping rule. The reported evaluation
count is the number of new image calculations estimated from archived manager
logs; wall time is a secondary metric because scheduler and filesystem noise
are not controlled.

\begin{table*}[t]
\centering
\caption{Convergence acceleration ablations. Each row starts from the same
archived chain and retains the same endpoint-defined path energy.}
\label{tab:acceleration}
\begin{tabularx}{\textwidth}{l l r r r r X}
\toprule
Case & Optimizer & Steps & Time (s) & Image evaluations & Final $f_{\max}$ & Result \\
\midrule
BTO n=9 & global FIRE & 28 & 2669 & 205 & 0.0967 & baseline \\
BTO n=9 & BlockFIRE, per-image 0.02 & 10 & 963 & 79 & 0.0865 & converged \\
BTO n=9 & BlockFIRE, per-image 0.01 & 5 & 478 & 44 & 0.0899 & fastest conservative transfer \\
BTO n=9 & ImageScaledFIRE & 8 & 772 & 65 & 0.0781 & converged \\
HfO$_2$ n=7 & global FIRE & 42 & 5242 & 217 & 0.0942 & baseline \\
HfO$_2$ n=7 & StagedFIRE & 24 & 3513 & 134 & 0.0924 & staged transfer \\
\bottomrule
\end{tabularx}
\end{table*}

The conservative BTO BlockFIRE row reduces the threshold time by 82.1\% and
estimated image evaluations by 78.5\% relative to global FIRE, while the
staged HfO$_2$ transfer reduces them by 33.0\% and 38.2\%, respectively. A
matched global-cap control and a SplitFIRE transfer are retained in the
project benchmark as negative controls. BlockFIRE plateaued on one 20-image
HfO$_2$ chain, so the result is not advertised as a universal speedup. The
reproducible claim is narrower: local state isolation and staged feasibility
control can reduce expensive work on these variable-cell paths.
...
Each no-DFT preflight records an ordered endpoint identity from species, PBC,
cell and wrapped fractional coordinates; a cross-calculator production gate
requires the initial and final identities to match the accepted reference
preflight separately. Thus a silent atom reorder cannot be misreported as a
backend comparison of the same path.
...
Calculators & ABACUS, VASP and QE ASE-facing factories & ABACUS and VASP have
completed material paths; QE has a static no-DFT preflight only \\
...
\subsection{VASP backend and cross-case benchmark}

The same core and image-level contract was exercised with VASP 6.3.2/PBE on
the Hefei Slurm system. Endpoints were not dispatched to workers during a
path; only the interior images were evaluated, with fixed PAW data, k-point
settings, $\mathrm{ISYM}=-1$, and $\mathrm{SYMPREC}=10^{-4}$. The default
maximum generalized-force threshold remained $0.10$ eV/\AA{}. Table~\ref{tab:vasp}
lists the completed paths archived with their source-data files. The GaN
four-phase comparisons use the conventional cells and divide cell energies by
the number of GaN formula units. The CdSe entry is the cell-mapping branch and
is reported in meV/atom because the published G-SSNEB figure is a small
barrier landscape.

\begin{table*}[t]
\centering
\caption{Completed VASP backend paths. The literature values are comparison
...
\midrule
GaN B4$\rightarrow$B1, tetragonal & 45.7 GPa & 29 (27) & 0.3385 eV/GaN & 0.34 eV/GaN & barrier and path shape agree \\
...
The two GaN B4$\rightarrow$B1 routes provide the strongest independent
backend benchmark: their barriers are within 0.002 and 0.005 eV/GaN of the
Qian et al. values, respectively \cite{Qian2013}. The B3$\rightarrow$B1
negative control is scientifically useful because the present diagonal mapping
produces one broad high barrier, whereas the reference chain contains three
connected events. This is a path-construction limitation, not a VASP failure.
...
Future work will complete cross-implementation comparisons,
