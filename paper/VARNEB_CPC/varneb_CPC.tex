...
The quantitative material results used in this manuscript are ABACUS/PBE.
The VASP backend has additionally exercised the complete distributed path,
restart, and lattice-contract workflow on GaN, but that path is retained as
backend-validation evidence because its barrier remains mapping-sensitive and
does not reproduce the literature value. This separation is important: a
...
VASP has a converged GaN path plus full-SCF contract regressions; QE has a static
no-DFT preflight only \\
