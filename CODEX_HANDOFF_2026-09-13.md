...
- failure report 现在额外包含保守的 `failure_category`（`scf_nonconvergence`、`timeout`、`mpi_failure`、`nonfinite_evaluation`、`invalid_cell` 或 `calculator_or_optimizer_error`），并由回归覆盖关键词边界；它仍不替代读取 ABACUS/VASP 原始 stdout，也不自动重试 line-search。
...
- 27678924 的线性 cell 插值对照仍在运行：截至本轮已完成 step 37，fmax 从 0.965655 降至 0.146653 eV/A，连续下降；最新 manifest 仍只记录 interior image 1--5 且无失败。必须继续观察多个完整 step，完成后再与 log-strain 7/9-image 结果比较，不因单次回弹提前取消，也不在该分支启动 CI。
