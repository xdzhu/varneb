...
- 截至 2026-09-14 02:24 左右，`27678406` 已完成 step 44（`fmax=0.136682 eV/A`，step 36 的 `0.098970` 后出现连续回弹，仍在既定 300-step 窗口内观察），`27678407` 已完成 step 28（`fmax=0.265483 eV/A`，从 step 0 持续下降）；两条作业仍 RUNNING，manifest 分别只记录 `[1,2,3]` 与 `[1,2,3,4,5,6,7]`，当前未见 ABACUS 错误 task。
...
- 截至 2026-09-14 02:24 左右，CI `27678507` 已完成 step 37（`fmax=0.045945 eV/A`）；step 15--37 的回弹/平台窗口已连续观察，最佳仍为 step 31（`0.045168 eV/A`）附近，manifest 记录 5 个内部 image；当前仍 RUNNING、无错误 task。按约定不因单次回弹停止，待完整窗口或作业终止后再决定是否从最佳快照续算。
